...
source "qemu" "rhel9-cis1" {
  accelerator      = "kvm"
  boot_command     = ["<esc><wait>", "vmlinuz initrd=initrd.img inst.ks=cdrom:sr1:rhel9-cis1-ks.cfg", "<enter><wait500>",
  ...
  boot_wait        = "5s"
  cpus             = "2"
  cd_files         = ["http/rhel9-cis1-ks.cfg"]
  cd_label         = "OEMDRV"
  ...
  iso_target_path  = "../iso/"
  iso_urls         = ["../iso/rhel-baseos-9.1-x86_64-dvd.iso"]
  memory           = "2048"
...
build {
  sources = ["source.qemu.rhel9-cis1"]
