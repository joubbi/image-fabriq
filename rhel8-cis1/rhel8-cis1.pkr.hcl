...
source "qemu" "rhel8-cis1" {
  accelerator      = "kvm"
#  boot_command     = ["<esc><wait>", "vmlinuz initrd=initrd.img inst.ks=cdrom:sr1:rhel8-cis1-ks.cfg", "<enter>"]
  boot_command     = ["<esc><wait>", "vmlinuz initrd=initrd.img inst.ks=cdrom:sr1:rhel8-cis1-ks.cfg", "<enter><wait500>",
  ...
  boot_wait        = "5s"
  cpus             = "2"
  cd_files         = ["http/rhel8-cis1-ks.cfg"]
  cd_label         = "OEMDRV"
  ...
  iso_target_path  = "../iso/"
  iso_urls         = ["../iso/rhel-8.7-x86_64-dvd.iso"]
  memory           = "2048"
...
build {
  sources = ["source.qemu.rhel8-cis1"]
