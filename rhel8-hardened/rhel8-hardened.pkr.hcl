...
source "qemu" "rhel8-hardened" {
  accelerator      = "kvm"
#  boot_command     = ["<up><wait><tab><wait> text ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rocky8-hardened-ks.cfg<enter><wait>"]
  boot_command     = ["<esc><wait>", "vmlinuz initrd=initrd.img inst.ks=cdrom:sr1:rhel8-hardened-ks.cfg", "<enter>"]
  boot_wait        = "5s"
  cpus             = "2"
  cd_files         = ["http/rhel8-hardened-ks.cfg"]
  cd_label         = "OEMDRV"
...
build {
  sources = ["source.qemu.rhel8-hardened"]
