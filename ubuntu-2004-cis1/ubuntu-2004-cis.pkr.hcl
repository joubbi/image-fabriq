...
build {
  sources = ["source.qemu.ubuntu-2004-cis"]

  provisioner "shell" {
    execute_command = "echo ${local.new_ssh_pass} | {{ .Vars }} sudo -S -E bash '{{ .Path }}'"
    scripts         = ["remediation-role.sh"]
  }
